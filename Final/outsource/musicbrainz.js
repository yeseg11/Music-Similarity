...
function query(options){    //make the link we need with all the modolar data (year,limit, offset and type)
	...
	return new Promise((resolve, reject)=>{
		switch(options.query){
			case 'ageYear':
				return getDataFromMB({url: `https://musicbrainz.org/ws/2/${options.type}?query=date:${options.date}&limit=${options.limit || 100}&offset=${options.offset || 0}&fmt=json`}).then(resolve).catch(reject)
			break;
		}
